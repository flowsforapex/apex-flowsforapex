...
- Provides underlying support for BPMN Adhoc SubProcesses (Enterprise Edition feature), allowing manual or AI-driven adhoc sub processes.
...
- Add database schema annotations for all tables and table columns for appropriate database versions.  Also for views and view columns (view annotations only supported with database 23ai+)
- All tasks now include a Description field for better human and AI understanding of their purpose.
- Fixes a bug that sometimes caused restarted errored subflows that subsequently error again to cause instance failures.
- Removes bundled copy of Monaco editor to reduce app size, instead relying on the Monaco editor already included in APEX.  
  - Note: On upgrading underlying APEX version, you might need to reconfigure the Component Settings for the Flows for APEX - Modeler plugin to set Monaco version matching your system.
- Includes the ORACLE-APEX-JSON-REGION plugin from Ewe Simon.
- Support for APEX v26.1. (Note: You must upgrade the Universal Theme to 26.1 before using the Flows for APEX app).
