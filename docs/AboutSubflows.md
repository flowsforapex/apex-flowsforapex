...
Subflows is a major change introduced in Flows for APEX release 4.0.

This documentation explains the subflow architecture, it's implementation, and how to interact with subflows to create low-code BPMN-based workflows using Flows for APEX.
...
The Process Start automatically calls a next_step event on the 'main' subflow.  In our example this steps the 'main' subflow to the next object, an opening (splitting) parallel gateway.

#### Standard Subflow Progression

Unless the subflow reaches a Gateway or an Event, the subflow progresses from object to object each time the user application signals that the current activity is completed by making a next_step call on the subflow.
...
- a new child subflow is created for each of the forward paths.  Each child subflow has a starting object set to the Gateway, last completed object set to the Gateway, and a next_step is performed to move it onto the first object in that subflow.
...
- when all of the child subflows originating from the opening parallel gateway have a status of `waiting at gateway`, the closing parallel gateway can continue.  Each of the child subflows is completed and then killed.  The original parent subflow, left behind at the starting parallel gateway with a status of `split`, has its status changed back to `running`, with its current object set to the merging gateway.  A next_step is then performed to move the parent subflow onto the next object in its sequence.
...
- the sub process subflow has completed and is killed
- the parent process task, which has been waiting, changes its status from `in subprocess` to `running`, and continues to opeate at the parent process level.
...
If there are no other subflows active, the Process Instance has completed and the Process has its process status set to `completed`.
