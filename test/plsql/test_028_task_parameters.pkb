...
is
  ...
  g_model_a28a constant varchar2(100) := 'A28a - Script Task Runtime Parameters';
  ...
  begin
    ...
    open l_expected for
      select flow_constants_pkg.gc_prcs_status_error as prcs_status
        from dual;

    open l_actual for
      select prcs_status
        from flow_processes
       where prcs_id = g_prcs_id_b;
  ...
  begin
    g_prcs_id_b := flow_api_pkg.flow_create
                   ( pi_dgrm_id   => g_dgrm_a25c_id
                   , pi_prcs_name => 'test 028 - disallow userInput'
                   );
    ...
    begin
      l_dummy := flow_parameters.process_input_parameters
                 ( pi_parameter_definitions => l_param_defs
                 , pi_user_input_data       => '{"customer_name":"Alice"}'
                 , pi_process_id            => g_prcs_id_b
                 , pi_scope                 => 0
                 , pi_allow_user_input      => false
                 );
    exception
      when others then
        l_raised := true;
    ...
    ut.expect( l_raised ).to_equal( true );
  end process_input_parameters_disallow_user_input;
  ...
  is
    ...
    l_expected_tstz   varchar2(128);
  begin
    g_prcs_id_e := flow_api_pkg.flow_create
                   ( pi_dgrm_id   => g_dgrm_a28a_id
                   , pi_prcs_name => 'test 028 - runtime input/output mapping'
                   );

    flow_process_vars.set_var
    ( pi_prcs_id   => g_prcs_id_e
    , pi_var_name  => 'fred'
    , pi_vc2_value => 'ProcVC2'
    );

    flow_process_vars.set_var
    ( pi_prcs_id   => g_prcs_id_e
    , pi_var_name  => 'fred_num'
    , pi_num_value => 77
    );

    flow_process_vars.set_var
    ( pi_prcs_id    => g_prcs_id_e
    , pi_var_name   => 'fred_date'
    , pi_date_value => to_date('2026-06-24 11:22:33','YYYY-MM-DD HH24:MI:SS')
    );

    flow_process_vars.set_var
    ( pi_prcs_id    => g_prcs_id_e
    , pi_var_name   => 'fred_tstz'
    , pi_tstz_value => to_timestamp_tz('2026-06-24 11:22:33.123456 +02:00','YYYY-MM-DD HH24:MI:SS.FF TZH:TZM')
    );

    flow_process_vars.set_var
    ( pi_prcs_id    => g_prcs_id_e
    , pi_var_name   => 'fred_clob'
    , pi_clob_value => 'ProcCLOB'
    );

    flow_api_pkg.flow_start( p_process_id => g_prcs_id_e );
    test_helper.step_forward( pi_prcs_id => g_prcs_id_e, pi_current => 'Activity_Pre' );

    select sbfl_task_input_parameters
         , sbfl_task_output_parameters
      into l_input_json
         , l_output_json
      from flow_subflows
     where sbfl_prcs_id = g_prcs_id_e
       and sbfl_current = 'Activity_Post';

    l_expected_date := to_char(to_date('2026-06-24 11:22:33','YYYY-MM-DD HH24:MI:SS'),'YYYY-MM-DD"T"HH24:MI:SS');
    l_expected_tstz := to_char(to_timestamp_tz('2026-06-24 11:22:33.123456 +02:00','YYYY-MM-DD HH24:MI:SS.FF TZH:TZM'),'YYYY-MM-DD"T"HH24:MI:SS.FFTZH:TZM');

    ut.expect( l_input_json ).to_be_like( '%"InputVC2Static":"InStaticVC2"%' );
    ut.expect( l_input_json ).to_be_like( '%"InputProcVarVC2":"ProcVC2"%' );
    ut.expect( l_input_json ).to_be_like( '%"InputProcVarNUM":77%' );
    ut.expect( l_input_json ).to_be_like( '%"InputProcVarDATE":"' || l_expected_date || '"%' );
    ut.expect( l_input_json ).to_be_like( '%"InputProcVarTSTZ":"' || l_expected_tstz || '"%' );
    ut.expect( l_input_json ).to_be_like( '%"InputProcVarCLOB":"ProcCLOB"%' );

    ut.expect( l_output_json ).to_be_like( '%"OutputVC2Static":"InStaticVC2"%' );
    ut.expect( l_output_json ).to_be_like( '%"OutputVC2ProcVar":"ProcVC2"%' );

    test_helper.step_forward( pi_prcs_id => g_prcs_id_e, pi_current => 'Activity_Post' );
  end runtime_maps_inputs_and_outputs;
