...
begin
  ...
  begin
    ...
    -- database 23.8 bug 36838600 requires that you add the column aliases isRole and role on the 3rd and 4th line of select list (or get ORA-600 [qolTextIdn:1])
    -- database 23.10 (aka 26.0) bug introduced while fixing 33561572 means we have to add an optimiser instruction using fix_control to disable 
    --          that fix to avoid another ORA-600 [qolTextIdn:1] - ( /*+ OPT_PARAM('_fix_control' '33561572:0') */ )
