...
alter table flow_process_variables add constraint prov_pk primary key (prov_prcs_id, prov_var_name);
 ...
 -- maybe we fold this into flow_object_attributes later, but use dedicated table during initial feature dev
 ...
 CREATE TABLE flow_object_expressions (
    ...
    expr_phase       VARCHAR2(4 CHAR) NOT NULL,
...
ALTER TABLE flow_object_expressions 
    ADD CONSTRAINT expr_uk UNIQUE ( expr_objt_id, expr_phase, expr_order);
...
ALTER TABLE flow_object_expressions
    ADD CONSTRAINT expr_phase_ck CHECK ( expr_phase in ('pre', 'post') );
