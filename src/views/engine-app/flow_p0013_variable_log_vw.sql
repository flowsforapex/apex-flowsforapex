...
begin
  if l_major >= 23 then
    execute immediate q'[alter view flow_p0013_variable_log_vw annotations
  ( add app     'Flows for APEX'
  , add type    'logging'
  , add content 'Variable change log with type-coalesced display value for engine app page 13'
  )]';
    execute immediate q'[alter view flow_p0013_variable_log_vw modify (lgvr_value annotations (add content 'Variable value coalesced across all typed columns as VARCHAR2'))]';
  end if;
end;
/
