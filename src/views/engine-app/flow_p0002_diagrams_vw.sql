...
as
  select d.dgrm_id
       ...
       , '<button type="button" title="View" aria-label="View" class="clickable-action t-Button t-Button--noLabel t-Button--icon" data-dgrm="' || d.dgrm_id || '" data-action="dgrm_view"><span aria-hidden="true" class="t-Icon fa fa-eye"></span></button>'||
           ...
           '<button type="button" title="Download" aria-label="Download" class="t-Button t-Button--noLabel t-Button--icon" onclick="'||apex_page.get_url(p_page => 5, p_items => 'P5_DGRM_ID', p_values => d.dgrm_id)||'"><span aria-hidden="true" class="t-Icon fa fa-download"></span></button>'||
