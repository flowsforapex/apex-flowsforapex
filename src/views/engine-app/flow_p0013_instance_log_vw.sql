...
alter view flow_p0013_instance_log_vw annotations
  ( add app     'Flows for APEX'
  , add type    'runtime'
  , add content 'Instance event log with event icons and error HTML formatting for engine app page 13'
  );

alter view flow_p0013_instance_log_vw modify (lgpr_prcs_event_icon annotations (add content 'FA icon CSS class representing the event type'));
alter view flow_p0013_instance_log_vw modify (pretag annotations (add content 'HTML pre/code open tag for error info display; null when no error'));
alter view flow_p0013_instance_log_vw modify (posttag annotations (add content 'HTML pre/code close tag for error info display; null when no error'));
